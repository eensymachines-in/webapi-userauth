...
func DuplicateAccount(ua IUsrAcc, db IMongoQry) (bool, error) {
	return false, nil
}

// RegisterNewAccount : registers a new account in the database and sends back the result
func RegisterNewAccount(ua IUsrAcc, db IMongoQry, result *map[string]interface{}) error {
	if ua == nil {
	...
	count, err := db.AddOneToColl(ua, "usraccs")
	if err != nil {
		return err
	}
	...
	*result = map[string]interface{}{
		// "id":    newAcc.Id.Hex(),
		"email": newAcc.Contact()["email"],
	} // sending back the result as output param
	return nil
}
